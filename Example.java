...
public class Example {
    ...
    /**
     * Takes a Character array of pentominoes then works through them and creates all the possible rows it can make
     * so that the rows can be worked on by the DLX.
     * @param pentominoes Simple Character array which holds the pentominoes we got from the user
     * @return The Sparse Matrix Which is used by the DLX. There is a cool concept I have followed here.
     * If we have n inputs then the format is -> (uniqueInputs)[X*n](PossiblePositions)[Y*row*col]
     * Say we have 2 pentominoes and 8 possible positions [2][4] array then format is -> XXYYYYYYYY
     * This makes sure that the DLX works for different 'same' inputs like 'U', 'X', 'U'
     */
    static int[][] makeSparseMatrix(Character[] pentominoes){
        ...
        //We use this, so we can use different types of pentominoes but also if there are
     ...
     * It just adds the specific ROW we just found to a group of rows. It does this by
     * creating that format I specified up, creates seperately the first part {XXX} and the second part {YYY}
     * then connects them as one singular row. Adds it to the hashmap and then adds it to the rows.
     *
     * @param localGrid simple local grid used to get the possible spaces restricted by piece at position
     * @param pentominoIndex keeps track of the index of the input pentominoes
     * @param row specific row got from above function, we use this data for the hashmap
     * @param col specific col got from above function, we use this data for the hashmap
     * @param mut mutation index
     * @param input_length number of pentominoes inputted
     * @param pentID pentomino ID
     *
     **/
    private static void addSpecificRowToRows(int[][] localGrid, int pentominoIndex, int row, int col, int mut, int input_length, int pentID) {
        ArrayList<Integer> theFirstN = new ArrayList<>();
        //Per each index put a 1 in its position so DLX can distinguish the different pieces,
     ...
     */
    public static boolean isPlacableForExample(int[][] field, int pentID, int mutation, int current_row, int current_col) {
    ...
    static void runExample() throws InterruptedException {
        ...
        //if there are more blocks in the field than in the pentominoes its impossible to solve
    ...
    /**
     * This gets the answer as used columns from the DLX per each row, so we need to translate it into readable form.
     * @param answers Is an arraylist of string which includes each row given by dlx and has inside of it words which contain
     *                the columns which were activated by that specific row. So what we do is go through our options and check which option
     *                can activate those columns. This is unique because of the identifier XXXYYY.
     */
    public static void ReturnPentominoesUsed(ArrayList<String> answers) throws InterruptedException {
        ...
        for (String curr_answer : answers){
            ...
            for (ArrayList<Integer> Row : theRows){
                ...
                //This means that the Row had all the positions and their possible size confirmed
    ...
    /**
     * Adds a piece to the field, puts the pieceID at the positions on the board which the piece corresponds.
    ...
    /**
     * Adds a piece to the field, puts a '1' at the positions on the board which the piece corresponds.
     * @param field the field to put the piece
     * @param piece the full piece
     * @param x row
     * @param y col
